...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__d('Accounts/authz', 'Menu'), ['action' => 'menu']) ?></li>
...
<div class="auditLogs index large-9 medium-8 columns content">
    <fieldset>
        <legend><?= __d('Accounts/authz','Authorization User &nbsp;::&nbsp; All Users') ?></legend>
...
<script>
    $(function() {
        ...
        $.ajax({
            url: '<?= Router::url(['plugin' => 'Accounts/Authz', 'controller' => 'AllUsers', 'action' => 'listAuthorizations']) ?>',
            beforeSend: function () {
        ...
        $('#add-button').click(function () {
            ...
            var dataString = 'plugin=' + plugin + '&controller=' + controller + '&action=' + action + '&prefix=' + prefix + '&extension=' + extension + '&access_type=' + access_type;
            $.ajax({
                type: 'GET',
                data: dataString,
                url: '<?= Router::url(['plugin' => 'Accounts/Authz', 'controller' => 'AllUsers', 'action' => 'addAuthorization']) ?>',
        ...
        loadController = (function() {
            $.ajax({
                url: '<?= Router::url(['plugin' => 'Accounts/Authz', 'controller' => 'AllUsers', 'action' => 'listCtls']) ?>' + '/' + encodeURIComponent($('#select-plugin').val()) + '.json',
                beforeSend: function () {
                    $('#div-controller').show();
        ...
        $('#select-controller').change(function() {
            ...
            $.ajax({
                url: '<?= Router::url(['plugin' => 'Accounts/Authz', 'controller' => 'AllUsers', 'action' => 'listActs']) ?>' +
                    '/' + encodeURIComponent($('#select-plugin').val()) +
                    '/' + encodeURIComponent($('#select-controller').val()) +
                    '.json',
                beforeSend: function( ) {
                    $('#div-action').show();
                    $('#select-action').empty();
